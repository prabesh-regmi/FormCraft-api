...
export default function ModalTemplateWithArgs({
  ...
  const handleOpenModal = (e) => {
    if (modalRef.current && !modalRef.current.contains(e.target)) {
      setTranslateClass("-translate-y-20");
      setTimeout(() => {
        onClose();
      }, 100);
    }
  ...
  return (
    <div
      ...
      tabIndex="-1"
      aria-hidden="true"
      className="fixed bg-black bg-opacity-50 top-0 left-0 right-0 bottom-0 z-[9999] w-full overflow-hidden md:inset-0 h-[100vh] flex justify-center"
    >
      <div
        className={`rounded-lg max-w-7xl max-h-[96vh] px-2 md:px-0 my-0 md:mx-6 xl:mx-0 my-[4vh] overflow-hidden ${
          width || "w-sm"
